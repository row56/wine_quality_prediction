...
tune_spline_df <- function(train_data, val_data, predictor, weights = NULL, title_suffix = "") {
    ...
    plot(1:length(df_values), huber, type = "b", xlab = "df",
...
test_results_simple <- evaluate_model(spline_simple, test,
...
test_results_weighted <- evaluate_model(spline_weighted, test,
...
test_results_mixed <- evaluate_model(spline_mixed_sampling, test,
...
test_results_mixed_weighted <- evaluate_model(spline_mixed_weighted, test,
